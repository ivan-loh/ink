...
#[test]
fn sync_push_updates_remote_when_only_frontmatter_title_changes() {
    let server = MockServer::start();
    let workspace = temp_workspace();
    ...
    .expect("encrypt note");
    let encrypted_note_after = encrypt_item_payload_004(
        &json!({"title": "Renamed Title", "text": "Fixture Body"}),
        items_key,
        note_uuid,
        None,
        None,
    ...
    let note_file = find_first_note_file(&workspace.path).expect("note file");
    let raw = fs::read_to_string(&note_file).expect("read note");
    let edited = raw.replacen("title: Fixture Title", "title: Renamed Title", 1);
    assert_ne!(
        edited, raw,
        "expected fixture frontmatter title to be updated in local file"
    );
    fs::write(&note_file, edited).expect("write edited note");
    ...
    let rendered = fs::read_to_string(&note_file).expect("read mirrored note");
    assert!(rendered.contains("title: Renamed Title"));
    assert!(rendered.contains("Fixture Body"));
...
#[test]
fn sync_status_auto_refreshes_expiring_session_on_command_entry() {
    let server = MockServer::start();
    let workspace = temp_workspace();

    init_workspace(&workspace.path, &server.base_url());
    seed_session_with_access_expiration(&workspace.path, &server.base_url(), 1);
    ...
    let refresh = server.mock(|when, then| {
        ...
        then.status(200).json_body(json!({
            "session": {
                ...
                "access_expiration": 4_102_450_000i64,
                "refresh_expiration": 4_102_453_600i64,
                "readonly_access": false
            }
        }));
    });

    let output = run_command(&workspace.path, &["sync", "status", "--json"]);
    assert_eq!(output["ok"], true);

    let refreshed = load_session(&workspace.path).expect("stored session");
    assert_eq!(refreshed.session.access_token, "access-token-fresh");
    assert_eq!(refreshed.session.refresh_token, "refresh-token-fresh");
...
fn seed_session(workspace: &Path, server_url: &str) {
    seed_session_with_access_expiration(workspace, server_url, 4_102_444_800);
}

fn seed_session_with_access_expiration(workspace: &Path, server_url: &str, access_expiration: i64) {
...
fn load_session(workspace: &Path) -> Option<StoredSession> {
    let paths = resolve_workspace(Some(workspace)).expect("resolve workspace");
    let sessions = SessionStore::from_workspace(&paths).expect("session store");
    sessions.load("default").expect("load session")
}
